...
function determineSteps(r){
  ...
  steps.push("expectations","legal","health","medication","treatment","income","goals","summary");
  return steps;
}
...
function stepHtml(step,r,a){
  ...
  switch(step){
    ...
    case "income": return `<section class="prescreen-step"><h3>Current Income Source</h3><p class="hint">Record the applicant’s current source of income. This information is for admission planning and can be updated later.</p>${radioGroup("incomeSource",[["employment","Employment","Currently earning employment income"],["odsp","ODSP","Ontario Disability Support Program"],["ow","Ontario Works","Ontario Works assistance"],["ei","Employment Insurance (EI)","Employment Insurance benefits"],["cpp","CPP / CPP-D","Canada Pension Plan or CPP Disability"],["pension","Pension / Retirement","Pension or retirement income"],["none","No Current Income","Applicant reports no current income"],["other","Other","Another income source"]],x.incomeSource)}<div class="prescreen-two-column">${field("incomeSourceName","Employer / source name","text",x.incomeSourceName||"","Employer, agency, pension, or other source")}${field("monthlyIncome","Approx. monthly income","number",x.monthlyIncome||"","Amount in dollars")}</div>${optionalTextarea("incomeNotes","Income notes",x.incomeNotes,"Additional income details or circumstances")}</section>`;
...
function summaryHtml(r,a){
  ...
  if(x.incomeSource) rows.push(["Current income",({employment:"Employment",odsp:"ODSP",ow:"Ontario Works",ei:"Employment Insurance (EI)",cpp:"CPP / CPP-D",pension:"Pension / Retirement",none:"No Current Income",other:"Other"}[x.incomeSource]||x.incomeSource)+(x.monthlyIncome?` — approx. $${x.monthlyIncome}/month`:"")]);
...
function validateStep(step,r){
  ...
  if(step==="income") return need(x.incomeSource,"Select the applicant’s current income source.",'input[name="incomeSource"]');
...
async function confirmPrescreenMoveRoster(){
  ...
  const sourcePrescreen=getRecord(applicant.id); const resident={id:crypto.randomUUID(),roomNumber:"",clientId:"",firstName:applicant.firstName||"",lastName:applicant.lastName||"",dob:applicant.dob||"",phone:applicant.contact||applicant.phone||"",address:applicant.address||"",city:applicant.city||"",contact:applicant.emergencyContact||"",contactPhone:applicant.emergencyContactPhone||"",entryDate,expectedDischargeDate:"",originalApplicationDate:applicant.originalApplicationDate||applicant.dateApplied||"",waitlistSourceId:applicant.id,preScreeningRecordId:sourcePrescreen?.id||applicant.preScreeningRecordId||"",preScreening:sourcePrescreen?structuredClone(sourcePrescreen):null,currentIncomeSource:sourcePrescreen?.answers?.incomeSource||"",currentIncomeSourceName:sourcePrescreen?.answers?.incomeSourceName||"",monthlyIncome:sourcePrescreen?.answers?.monthlyIncome||"",incomeNotes:sourcePrescreen?.answers?.incomeNotes||"",phase:"phase1",phase2AdmissionDate:"",archived:false,archivedAt:"",archiveReason:"",notes:[note,...(Array.isArray(applicant.notes)?applicant.notes:[])],activityHistory:normalizeActivityHistory(applicant.activityHistory)};
